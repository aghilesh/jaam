            ...
            <tbody id="eduRows">
                ...
                if($_POST && array_key_exists('edu', $_POST)) {
                    ...
                    for($i = 0; $i < sizeof($enquiryEducation); $i++ ) {
                        ?>
                       <tr>
                            <td><input class="generic-input validate validate-mandatory" type="text" name="edu[qualification][<?php echo $i?>]" placeholder="Qualification" value="<?php echo $enquiryEducation[$i]->qualification?>"/></td>
                            <td><input class="generic-input validate validate-mandatory" type="text" name="edu[university][<?php echo $i?>]" placeholder="Board/University" value="<?php echo $enquiryEducation[$i]->board?>"/></td>
                            <td><input class="generic-input validate validate-mandatory" type="text" name="edu[institution][<?php echo $i?>]" placeholder="Institution" value="<?php echo $enquiryEducation[$i]->institution?>"/></td>
                            <td><?php echo form_dropdown('edu[country_id]['.$i.']', $countries,$enquiryEducation[$i]->country_id,'class="generic-input"');?></td>
                            <td><input class="generic-input validate validate-mandatory" type="text" name="edu[passoutyear][<?php echo $i?>]" placeholder="Passout Year" value="<?php echo $enquiryEducation[$i]->passout_year?>"/></td>
                            <td><input class="generic-input validate validate-mandatory  edu-percentage" type="text" name="edu[percentage][<?php echo $i?>]" placeholder="Percentage" value="<?php echo $enquiryEducation[$i]->percentage?>"/></td>
                            <td>
                                <?php 
                                $actions =  array("select"=>"Select","remove"=>"Remove","add"=>"Add");
                                echo form_dropdown('action', $actions,'','class="generic-input" onchange="Enquiry.eduAction(this)" ');?>
