...
class Country extends CI_Controller {
    ...
    public function add() {
        if ($_POST) {
            ...
            $this->form_validation->set_rules('currency', 'Currency', 'required|xss_clean|max_length[100]');
            $this->form_validation->set_rules('currency_name', 'Currency Name', 'xss_clean|max_length[100]');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->country->insert($formData)) {
                    $this->session->set_flashdata('message', 'Country was added successfully.');
                    $this->session->set_flashdata('msg_class', 'success_message');
                    redirect('country');
                } else {
                    $this->session->set_flashdata('message', 'There was some problem in adding the Country.');
                    $this->session->set_flashdata('msg_class', 'error_message');
                    redirect('country/add');
                }
            }
        ...
        $this->gen_contents['page_title'] = 'Country - Add';
    ...
    public function edit() {
        ...
        if ($_POST && $id) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->country->update($formData, $id) == TRUE) {
                    $this->session->set_flashdata('message', 'Country was updated successfully.');
                    $this->session->set_flashdata('msg_class', 'success_message');
                    redirect('country/edit/' . $id);
                } else {
                    $this->session->set_flashdata('message', 'There was some problem in updating the Country.');
                    $this->session->set_flashdata('msg_class', 'error_message');
                    redirect('country/edit/' . $id);
                }
            }
        ...
        $this->gen_contents['page_title'] = 'Country - Edit';
    ...
    public function delete($id) {
        ...
        if ($id) {
            if ($this->country->delete($id)) {
                $this->session->set_flashdata('message', 'Country was deleted successfully.');
                $this->session->set_flashdata('msg_class', 'success_message');
                redirect('country');
            } else {
                $error = true;
